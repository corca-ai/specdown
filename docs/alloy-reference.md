...
(section [Lexical Issues](#lexical-issues)).
...
[Commands](#commands) below). The length of explored
...
[Signature Declarations](#signature-declarations)). A similar treatment applies
to multiplicity constraints (see sections
[Multiplicity Keywords](#multiplicity-keywords) and
[Multiplicities](#multiplicities)) and `disj`. In this
...
[Integer Expressions](#integer-expressions). The `disj`
...
As explained in section [Semantic Basis](#semantic-basis),
...
[Expression Paragraphs](#expression-paragraphs)), but are treated as
...
section [Boolean Expressions](#boolean-expressions)). The
...
[Signature Declarations](#signature-declarations)). If there is more than one
field of the given name, the reference is resolved, or rejected if
ambiguous (see section
[Types and Overloading](#types-and-overloading)). If the name
...
(section [Module Structure](#module-structure)). In this case, the name may
...
[Expression Paragraphs](#expression-paragraphs)). Field names cannot
...
[Expression Paragraphs](#expression-paragraphs).
...
explained in section [Multiplicities](#multiplicities).
...
(see section [Types and Overloading](#types-and-overloading)).
...
section [Commands](#commands), in an analysis the bitwidth
...
[Expression Paragraphs](#expression-paragraphs)) returns a boolean
value.
...
As explained in section [Declarations](#declarations), a
...
[Declarations](#declarations)). In a sequence of
