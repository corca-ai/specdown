...
1. Create a new `.spec.md` file. One file per feature ([best practice](../specs/best-practices.md#keep-documents-focused)).
...
If the project uses [traceability](../specs/traceability.md), add the appropriate trace links (e.g., `[covers::Feature](feature.spec.md)` in the parent goal document).
...
See [Doctest Blocks](../specs/syntax.md#doctest-blocks) for full syntax.
...
This renders as a green/red badge in the HTML report without interrupting the prose flow. See [Inline expect](../specs/syntax.md#inline-expect).

### Add check tables for repeated patterns

If you see three or more `run:shell` blocks testing variations of the same thing, refactor into a [check table](../specs/syntax.md#check-tables):
...
This requires an [adapter check](../specs/adapter-protocol.md), but the spec becomes pure data.
...
When the state space is too large for examples, add an [Alloy model](../specs/alloy.md) in the same section as the implementation checks. Use models for:
...
See [Alloy patterns](../specs/best-practices.md#patterns) for worked examples.

### Add traceability

If the project has goals or feature documents, add [trace edges](../specs/traceability.md) to ensure coverage. Run `specdown trace -strict` to find gaps.
...
2. **Determine the cause**:
   ...
   - **Environment issue** — missing dependency, stale state, ordering problem. Fix setup/teardown or add [hooks](../specs/syntax.md#setup-and-teardown-hooks).
...
Only one filter can be active at a time. See [Filter](../specs/cli.md#filter).
...
- **Add summary lines** to long shell blocks so the report stays scannable. See [Summary Lines](../specs/syntax.md#summary-lines).
