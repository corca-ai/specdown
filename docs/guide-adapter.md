...
3. specdown이 `runCase`를 문서 순서대로 보낸다
4. adapter가 각 case에 `caseStarted` → `casePassed` 또는 `caseFailed`로 응답한다
...
| `source` | 블록 본문 (변수 미치환 원본) |
| `bindings` | `[{"name": "x", "value": "1"}, ...]` |
| `captureNames` | `["userId"]` — 결과로 돌려줄 변수 이름 |

변수 치환은 adapter가 직접 한다. `${name}`을 bindings에서 찾아 치환하면 된다.
