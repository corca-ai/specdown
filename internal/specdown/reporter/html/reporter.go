...
var tocEntryTmpl = `
          <section class="toc-spec{{ if .Current }} current{{ end }}">
            <div class="toc-spec-header">{{ if .DocType }}<span class="toc-type-badge" style="--type-hue:{{ typeHue .DocType }}">{{ .DocType }}</span>{{ end }}{{ if .Current }}<span class="toc-spec-title {{ .Status }}">{{ .Title }}</span>{{ else }}<a class="toc-spec-title {{ .Status }}" href="{{ .Href }}">{{ .Title }}</a>{{ end }}</div>
