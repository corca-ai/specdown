...
- [CLI](${CLAUDE_SKILL_DIR}/cli.md) — Commands (`run`, `trace`, `init`, `alloy explore`, `alloy dump`, `install skills`), flags (`-config`, `-filter`, `-quiet`, `-jobs`, `-max-failures`), and filter expressions (`type:`, `block:`, `check:` prefixes). Read this when you need to understand commands or flags.
...
- [Alloy Explore Guide](${CLAUDE_SKILL_DIR}/guide-alloy-explore.md) — Iterative spec authoring with `specdown alloy explore`: grow Alloy models and implementation tests together by exploring instances, discovering surprises, and turning each finding into a concrete test. Read this when using Alloy to develop or improve specs.
